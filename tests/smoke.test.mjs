...
test('PWA metadata and worker quote boundary stay valid', () => {
  ...
  assert.equal(manifest.start_url, '/?v=60');
  assert.equal(manifest.background_color, '#f5f6f3');
  assert.match(serviceWorker, /wealth-v60/);
  assert.match(serviceWorker, /暂时无法连接/);
  assert.match(serviceWorker, /Navigation timeout/);
  assert.match(serviceWorker, /cache\.put\('\/', response\.clone\(\)\)/);
  assert.match(appMarkup, /register\('\/sw\.js\?v=60',\{updateViaCache:'none'\}\)/);
...
test('mobile drawer is explicit, scroll-safe, and uses vector icons', () => {
  ...
  assert.match(appMarkup, /#qaFab\.qa-fab\{left:50%!important;right:auto!important;bottom:calc\(36px \+ env\(safe-area-inset-bottom,0px\)\)!important;transform:translateX\(-50%\)!important/);
