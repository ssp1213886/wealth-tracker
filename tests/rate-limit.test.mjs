...
test('rate limiter allows 240 requests and blocks the next', () => {
  let now = 1000;
  const limiter = createRateLimiter({ now: () => now });
  for (let i = 0; i < 240; i++) assert.equal(limiter.check('ip'), true);
...
test('rate limiter resets after the window', () => {
  let now = 1000;
  const limiter = createRateLimiter({ now: () => now });
  for (let i = 0; i < 240; i++) limiter.check('ip');
  now += 61000;
...
test('rate limiter tracks clients independently', () => {
  let now = 1000;
  const limiter = createRateLimiter({ now: () => now });
  for (let i = 0; i < 240; i++) limiter.check('first');
