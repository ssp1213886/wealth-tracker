...
| 数据 | 现金管理（入金/出金/修正/股息）、持仓明细（含张数列+列排序）、交易历史（筛选/排序）、资金流水（权利金蓝色+股息橙色独立标注）、年度收益归因 |
| 日志 | 操作日志（热力图）、纪律打卡（连续定投+热力图）、年度复盘矩阵（4×5热力 权利金贡献率）、投资规划（收入渐进加码/退出策略/提款模拟） |
...
- 年度复盘矩阵：4行×5列热力方格，年度权利金贡献率（CC权利金÷总买入）驱动着色，含年末持仓 / CC权利金 / 总买入统计，右侧长线数据面板（总入金/总资产/CAGR/目标差额）
...
- 股息记录：VGT/SMH 股息入账（仅记现金流水，不混入月度定投指标）
