...
if('serviceWorker' in navigator){var swRefreshing=false;navigator.serviceWorker.addEventListener('controllerchange',function(){if(swRefreshing)return;swRefreshing=true;location.reload()});navigator.serviceWorker.register('/sw.js?v=86',{updateViaCache:'none'}).then(function(reg){return reg.update()}).catch(function(){})}
...
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',initSidebarMarketCollapse)}else{initSidebarMarketCollapse()}
...
if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',initSidebarHealthCollapse)}else{initSidebarHealthCollapse()}
