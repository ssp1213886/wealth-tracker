...
if('serviceWorker' in navigator){var swRefreshing=false;navigator.serviceWorker.addEventListener('controllerchange',function(){if(swRefreshing)return;swRefreshing=true;location.reload()});navigator.serviceWorker.register('/sw.js?v=74',{updateViaCache:'none'}).then(function(reg){return reg.update()}).catch(function(){})}
...
function recordSyncSuccess(direction,timestamp){var s=loadSyncState();s.lastSyncAt=Number(timestamp)||Date.now();s.lastSyncDirection=direction||'check';s.lastSyncErrorAt=0;s.lastSyncError='';saveSyncState(s);renderSyncHealth()}
function recordSyncFailure(timestamp,message){var s=loadSyncState();s.lastSyncErrorAt=Number(timestamp)||Date.now();s.lastSyncError=String(message||'').slice(0,200);saveSyncState(s);renderSyncHealth()}
...
syncFetch=function(method,body){var ms=document.getElementById('msSyncText'),host=ms&&ms.closest('.ms-sync');if(host)host.classList.add('is-syncing');var done=function(){if(host)host.classList.remove('is-syncing')};return syncFetchWithoutHealth(method,body).then(function(result){recordSyncSuccess(method==='GET'?'pull':'push',Date.now());done();return result}).catch(function(error){recordSyncFailure(Date.now(),error&&error.message);if(error&&console&&console.warn)console.warn('[sync]',error.message);done();throw error})};
