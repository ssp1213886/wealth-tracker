...
syncFetch=function(method,body){var ms=document.getElementById('msSyncText'),host=ms&&ms.closest('.ms-sync');var clearSpin=function(){if(host)host.classList.remove('is-syncing')};if(host)host.classList.add('is-syncing');setSyncBar('busy',method==='GET'?'正在下载…':'正在上传…');var spinTimer=setTimeout(clearSpin,12000);var done=function(){clearTimeout(spinTimer);clearSpin()};try{return syncFetchWithoutHealth(method,body).then(function(result){if(method!=='GET'&&result&&result.ts&&body){try{Object.keys(body).forEach(function(k){if(k!=='__expectedVersions')setCloudTs(k,result.ts)})}catch(e){}}recordSyncSuccess(method==='GET'?'pull':'push',Date.now());setSyncBar('ok','已同步 '+syncClockText());done();return result}).catch(function(error){recordSyncFailure(Date.now(),error&&error.message);if(error&&console&&console.warn)console.warn('[sync]',error.message);if(error&&error.status===409)setSyncBar('busy','正在核对…');else setSyncBar('err','同步失败');done();throw error})}catch(e){done();throw e}};
