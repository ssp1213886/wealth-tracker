...
import {buildSyncPayload, classifySyncError, normalizeSyncTs, syncContentEqual} from './sync.js';
...
function recordSyncSuccess(direction,timestamp){var s=loadSyncState();s.lastSyncAt=Number(timestamp)||Date.now();s.lastSyncDirection=direction||'check';if(s.lastSyncDirection==='push')s.lastPushAt=s.lastSyncAt;s.lastSyncErrorAt=0;s.lastSyncError='';s.failStreak=0;saveSyncState(s);renderSyncHealth()}
...
function pushKeysForce(keys,label){if(!keys||!keys.length)return;if(!syncCfg||!syncCfg.url||!syncCfg.token)return;var payload={};keys.forEach(function(k){payload[k]=localValOf(k)});setSyncBar('busy','正在用本机数据覆盖云端…');syncFetchWithoutHealth('POST',payload).then(function(r){var st=loadSyncState();keys.forEach(function(k){clearDirty(k);if(r&&r.ts)setCloudTs(k,r.ts)});st.pendingConflicts=[];st.lastSyncErrorAt=0;st.lastSyncError='';st.failStreak=0;saveSyncState(st);setSyncConflicts([]);setSyncBar('ok','已用本机数据覆盖云端 · '+syncClockText());showToast(label||'已用本机数据覆盖云端','ok')}).catch(function(e){setSyncBar('err','覆盖失败：'+((e&&e.message)||'未知错误'))})}
function healPushConflict(data){var sent=pushedKeysOf(data);if(!syncCfg||!syncCfg.url||!syncCfg.token){autoPull();return}setSyncBar('busy','云端有更新，正在自动核对…');syncFetchWithoutHealth('GET').then(function(r){if(!r||!r.data){setSyncBar('');autoPull();showToast('云端已有更新，请确认冲突','err');return}var meta=r.meta||{},st=loadSyncState(),diff=[];sent.forEach(function(k){var lv=localValOf(k),cv=r.data[k];if(cv===undefined||cv===null){delete st.cloudTs[k];st.dirty[k]=true;return}if(syncContentEqual(k,lv,cv,normalizeState)){st.dirty[k]=false;var ts=normalizeSyncTs(meta[k]);if(ts)st.cloudTs[k]=ts}else{diff.push(k)}});saveSyncState(st);if(diff.length){setSyncBar('');autoPull();showToast('云端已有更新，请确认冲突','err')}else{setSyncBar('ok','已自动对齐云端 · '+syncClockText());showToast('云端内容与本机一致，已自动对齐','ok')}}).catch(function(){setSyncBar('');autoPull();showToast('云端已有更新，请确认冲突','err')})}
